...
void setup() {
  ...
  WiFi.mode(WIFI_STA); WiFi.persistent(false); WiFi.setSleep(false); WiFi.setHostname(cameraHost()); WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  while (WiFi.status() != WL_CONNECTED) delay(250);
  if (MDNS.begin(cameraHost())) MDNS.addService("http", "tcp", 80);
  startServer();
  Serial.printf("%s ready: http://%s.local/stream\n", cameraName(), cameraHost());
}

void loop() { if (WiFi.status() != WL_CONNECTED) WiFi.reconnect(); delay(1000); }
